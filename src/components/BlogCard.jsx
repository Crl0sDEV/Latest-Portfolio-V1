...
export default function BlogCard({ post, index }) {
  ...
  return (
    <Reveal delay={0.1 * index}>
      <article 
        className={`relative p-6 text-left bg-zinc-900/60 backdrop-blur-md border border-white/10 rounded-2xl shadow-xl transition-all duration-300 hover:shadow-green-500/20 hover:border-green-500/30 flex flex-col ${isExpanded ? 'h-auto' : 'h-full'}`}
      >
        {/* Header Section */}
        <div className="mb-4">
        ...
        <div className={`prose prose-invert prose-sm max-w-none text-gray-400 leading-relaxed ${!isExpanded ? 'line-clamp-4' : ''}`}>
           {isExpanded ? (
              <ReactMarkdown 
                ...
                components={{
                    h1: ({node, ...props}) => <h3 className="text-lg font-bold text-green-200 mt-6 mb-3" {...props} />,
                    h2: ({node, ...props}) => <h4 className="text-base font-bold text-green-200 mt-5 mb-2" {...props} />,
                    p: ({node, ...props}) => <p className="mb-3 text-gray-300" {...props} />,
                    li: ({node, ...props}) => <li className="ml-4 list-disc marker:text-green-500" {...props} />,
                    ...
                    code({node, inline, className, children, ...props}) {
                        ...
                        return !inline && match ? (
                          ...
                          <code className="bg-green-500/10 text-green-300 px-1.5 py-0.5 rounded font-mono text-xs border border-green-500/20" {...props}>
                            {children}
                          </code>
                        );
                    }
        ...
        <div className="mt-auto pt-6 flex justify-center">
            ...
            >
                ...
                <span className={`absolute -bottom-8 left-1/2 -translate-x-1/2 w-1 h-1 bg-green-500 rounded-full transition-all duration-300 ${isExpanded ? 'opacity-100' : 'opacity-0'}`}></span>
            </button>
        </div>
